...
function addScrambles(discipline){



	var scramblesDiv = document.getElementById("add_scrambles_" + discipline);
	scramblesDiv.innerHTML = '<textarea id="scrambles_' + discipline + '"  name = "scrambles_' + discipline + '" placeholder = "Please separate scrambles with comma" rows = "5" cols = "55"></textarea>';
	scramblesDiv.innerHTML = scramblesDiv.innerHTML  + '</br>Generate <input type = "text" id = "numOfScrambles_' + discipline + '" value = "1" size = 1>random scrambles.';
	scramblesDiv.innerHTML = scramblesDiv.innerHTML  + '</br><button type="button" onclick = randomScrambles("' + discipline + '",1)>Generate</button>';

}

function randomScrambles(discipline, numOfScrambles){

	var num = document.getElementById("numOfScrambles_" + discipline).value;
	var dis = discipline.charAt(0);

	//TODO
	//generate num scrambles for dis discipline and add them to textarea.
	console.log("num "+ num + " dis " + dis);
	...
	var colorList=new Array(
	   ...
	   'p', "purple", "purple",
	   '0', "gray",   "grey"      // used for unrecognised letters, or when zero used.used for unrecognised letters, or when zero used.
	);
	...
	scramble();
	document.getElementById('scrambles_' + discipline).innerHTML = "";
	for( var i=0; i<numcub; i++){
	   document.getElementById('scrambles_' + discipline).innerHTML += scramblestring(i).replace(/<\/?[^>]+(>|$)/g, "") + ',\n';  
	}
	document.getElementById('scrambles_' + discipline).innerHTML = document.getElementById('scrambles_' + discipline).innerHTML.substring(0, document.getElementById('scrambles_' + discipline).innerHTML.length - 2);  
