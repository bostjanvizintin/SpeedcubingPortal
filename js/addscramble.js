...
function keyPressed(e){
   if (e.keyCode == "32" && !count){
      currentScramble = document.getElementById("scramble").innerHTML;
      addScramble();
   }
    count = !count;
}
...
function addScramble(){
   var selectedPuzzle = document.getElementById("puzzleSelect");
   size = selectedPuzzle.options[selectedPuzzle.selectedIndex].text.charAt(0);
   numcub = 1;
   scramble();
   var scrambleDiv = document.getElementById("scramble");
   var scr = "";
   for( var i=0; i<numcub; i++){
      scr +=scramblestring(i);
   }
   scrambleDiv.innerHTML = scr;
}
